...
class OwnHomeDataMessage extends PiranhaMessage {
  ...
  encode () {
    ...
    this.writeVInt(this.player.high_trophies)//HTRP
    this.writeVInt(this.player.high_trophies)//HTRP
    this.writeVInt(this.player.trophyroad_reward)//TrophyRoadReward
    this.writeVInt(this.player.experience)//EXP
    ...
    this.writeVInt(this.player.unlocked_skins.length)
    ...
    this.writeVInt(this.player.bp_season) // Season
    this.writeVInt(this.player.bp_tokens) // Tokens
    this.writeVInt(this.player.bp_activated) // isBrawlPass
    this.writeVInt(0)
    this.writeVInt(0)
    // BrawlPass array End
    ...
    this.writeDataReference(15, this.player.events_map1)  // MapID
    ...
    this.writeVInt(2)
    this.writeVInt(2)  // EventType
    this.writeVInt(0)  // EventsBeginCountdown
    this.writeVInt(0)  // Timer
    this.writeVInt(0)  // tokens reward for new event
    this.writeDataReference(15, this.player.events_map2)  // MapID
    this.writeVInt(-64)  // GameModeVariation
    this.writeVInt(2)  // State
    this.writeString()
    this.writeVInt(0)
    ...
    this.writeLongInt(1, 41000000 + this.player.theme_id) // ThemeID
    if (this.player.isSupportDisabled == true){
    this.writeLongInt(61, 36270) // SupportDisabled State | if 36218 < state its true
    }else{
      this.writeLongInt(61, 36218)
    }
    this.writeLongInt(29, this.player.activeSkinGroup) // Skin Group Active For Campaign | 10 enable | 5 Bad Randoms 7 Hallowen 10 Brawlywood
    ...
    this.writeVInt(this.player.id) //ID?
    this.writeVInt(this.player.id) //ID?
    ...
    this.writeString(this.player.username)
    this.writeVInt(1)
